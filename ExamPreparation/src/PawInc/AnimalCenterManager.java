...
public class AnimalCenterManager {
    ...
    static void sendForCleansing(String adoptionCenterName, String cleansingCenterName){
        List<Animal> dirtyAnimal = adoptionCenters.get(adoptionCenterName).getStoredAnimals();
...
//        adoptionCenters.get(adoptionCenterName).clean();
    }
    ...
    static void printStatistics(){
        ...
        sb.append("Adopted Animals: ").append(String.join(", ", adoptedAnimals.stream().map(Animal::toString).collect(Collectors.toList()))).append(System.lineSeparator());
        sb.append("Cleansed Animals: ").append(String.join(", ", cleansedAnimals.stream().map(Animal::toString).collect(Collectors.toList()))).append(System.lineSeparator());
        ...
        sb.append(String.format("Animals Awaiting Adoption: %s%n", awaitingAdoption > 0? awaitingAdoption:"None"));
        sb.append(String.format("Animals Awaiting Cleansing: %s%n", awaitingCleansing > 0? awaitingAdoption:"None"));
